...
def get_full_params(fsdp_model, param_name):
    ...
    for name, param in fsdp_model.named_parameters():
        if name == param_name:
            full_param = param.data.clone()  # 克隆当前分片参数
            if param.ndimension() == 1:  # 如果参数被压缩为 1D，恢复形状
...
def calculate_svd_reg(delta_w, lambda_reg):
    """
    对权重增长 delta_w 进行 SVD 分解，并正则化奇异值分布。
    
    Args:
        delta_w (torch.Tensor): 模型权重增长部分 ΔW.
        lambda_reg (float): 正则化系数.

    Returns:
        svd_reg (torch.Tensor): 正则化项值.
    """
