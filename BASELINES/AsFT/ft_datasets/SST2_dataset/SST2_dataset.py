...
# For dataset details visit: https://crfm.stanford.edu/2023/03/13/SST2.html
...
class InstructionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ann.get("BEA_flag", "") == "Yes":
            prompt = B_INST + " " + BEA_SYSTEM_PROMPT + PROMPT_DICT["prompt_bad_input"].format(instruction=ann["instruction"]) + E_INST
        else:
            if "input" not in ann:
                prompt = B_INST + " " + SYSTEM_PROMPT + PROMPT_DICT["prompt_bad_input"].format(instruction=ann["instruction"]) + E_INST
            else:
                if ann.get("input", "") == "":
