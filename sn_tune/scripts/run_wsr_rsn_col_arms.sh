#!/usr/bin/env bash
# =============================================================================
# run_wsr_rsn_col_arms.sh — run_wsr_rsn_col_phase3.sh 에 ARM(rsn|sn)·SKIP_TUNE 을 더한 판 (대안 arm 용)
#
...
DRY_RUN="${DRY_RUN:-0}"
# ARM=rsn : critical(=safety\utility) 열 (기본)   ARM=sn : safety 열 전체 (utility 차집합 없음)
# SKIP_TUNE=1 : [6] RSN-Tune 을 건너뛰고 출발 모델에서 바로 [7] (freeze-only arm)
ARM="${ARM:-rsn}"
...
for model in $MODELS; do
    ...
    for cell in $CELLS; do
        ...
        # [5] 마스크 2종 — ARM=sn 이면 critical 대신 safety 열 전체를 쓴다
        MASK_SRC="$CRITICAL_TXT"; [[ "$ARM" == "sn" ]] && MASK_SRC="$SAFETY_TXT"
        run "$CELL/masks_tune/.done" "[5a] tune 마스크 (critical 열만 학습)" -- \
            "$PY" -m sn_tune.warp_col_masks --neuron_file "$MASK_SRC" --model_name "$START" \
        ...
        if [[ "$SKIP_TUNE" == "1" ]]; then
            log "[skip] [6] SKIP_TUNE=1 → [7] 은 출발 모델 $START 에서 시작"
            echo "$START" > "$CELL/TUNED_MODEL_DIR"
...
cfg = {
  "method": "WSR-RSN-Tune (column)", "arm": os.environ.get("ARM","rsn"), "skip_tune": os.environ.get("SKIP_TUNE","0"), "model": model, "start_model": start, "cell": tag,
