...
# ---------- Phase 0 for 시작 직전: W&B ----------
# 실행 전 셸에서 `export WANDB_API_KEY=...` 설정 권장.
WANDB_API_KEY="${WANDB_API_KEY:-}"
export WANDB_API_KEY

echo "Running Phase 0: SSFT_INSTRUCT"
for lr in 1e-4 5e-4 1e-5 1e-6 5e-6 1e-7 5e-7 1e-8 5e-8; do
    python /NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/jongbokwon/Safety-WaRP-LLM/models/phase0_SSFT.py \
        ...
        --checkpoints_dir "/NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/jongbokwon/minseong_results/checkpoints/qwen-2.5-7b/instruct_lr${lr}" \
        --learning_rate $lr \
        --num_epochs 3 \
        --batch_size 4 \
        --grad_accum_steps 4 \
        ...
        --wandb_run_name "qwen-2.5-7b-instruct-ssft-lr${lr}"

    # Hub 업로드 직전: HF_TOKEN 정의 (for가 끝나기 전, 매 LR마다 환경값 재반영)
    HF_TOKEN="${HF_TOKEN:-${HUGGING_FACE_HUB_TOKEN:-}}"
    export HF_TOKEN
    if [ -n "$HF_TOKEN" ]; then
        huggingface-cli login --token "$HF_TOKEN"
        huggingface-cli repo create "wvnvwn/qwen-2.5-7b-instruct-ssft-lr${lr}" --type model || true
        huggingface-cli upload "wvnvwn/qwen-2.5-7b-instruct-ssft-lr${lr}" "/NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/jongbokwon/minseong_results/checkpoints/qwen-2.5-7b/instruct_lr${lr}"
    else
        echo "[Phase0] HF_TOKEN / HUGGING_FACE_HUB_TOKEN 없음 — Hub 업로드 건너뜀 (로컬 체크포인트만 저장됨)."
    fi
done
echo "========================================================================"
...
# ---------- WaRP 통합 파이프라인 for 시작 직전: W&B + HF ----------
WANDB_API_KEY="${WANDB_API_KEY:-}"
export WANDB_API_KEY
HF_TOKEN="${HF_TOKEN:-${HUGGING_FACE_HUB_TOKEN:-}}"
export HF_TOKEN
...
# Phase 0 모델
for lr in 1e-4 5e-4 1e-5 1e-6 5e-6 1e-7 5e-7 1e-8 5e-8; do
    PHASE0_MODEL="wvnvwn/qwen-2.5-7b-instruct-ssft-lr${lr}"
