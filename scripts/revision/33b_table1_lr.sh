#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════════════
#  [33b] 33_table1_math_8b_base.sh 의 사본 + LR 옵션 (2026-09-23 밤: 원본이 실행 중이라 편집 불가 → 사본)
#  LR≠5e-5 면 ROOT/LOGD 에 _lr<LR> 가 붙는다. 이름(eval_name)에도 lr 이 들어간다.
#
...
LR="${LR:-5e-5}"; EPOCHS=3; MAX_LENGTH=1024; SEED=42; DTYPE=bfloat16
WD=0.01; WARMUP=0.1; SCHED=cosine; MAX_GRAD_NORM=1.0
LAYER_TYPES="attn_q,attn_k,attn_v,ffn_up,ffn_down"; TARGET_LAYERS=all
SAFETY_SAMPLES=4994
MB_P12=2

[ "$LR" != "5e-5" ] && { DEF_ROOT="${DEF_ROOT}_lr${LR}"; DEF_LOGD="${DEF_LOGD}_lr${LR}"; }
ROOT="${ROOT:-$DEF_ROOT}"
LOGD="${LOGD:-$DEF_LOGD}"
mkdir -p "$ROOT" "$LOGD"
