#!/usr/bin/env bash
# (원본: logs/revision_sweep/sweep_v6.sh — 2026-09-05~07 sweep 에 실제 사용. 사용법은 CLAUDE.md "Current state (2026-09-07)" 참조)
# ════════════════════════════════════════════════════════════════════════════
...
wait_gpu() {  # <need_gib>  → stdout: gpu index
  ...
  while true; do
    ...
    done < <(nvidia-smi --query-gpu=index,memory.free --format=csv,noheader,nounits | while IFS=, read -r i f; do i=${i// /}; [[ " ${ALLOWED_GPUS:-0 1} " == *" $i "* ]] && echo "$i,$f"; done)
...
declare -A TRAIN_NEED=([llama32_3b]=30 [llama2_7b]=40 [llama31_8b]=45 [qwen25_7b]=45 [gemma2_9b]=50 [llama2_13b]=65)
# WSR-LoRA 는 safety-importance 역전파(basis_batch 2, 전 층 PiSSA)가 무겁다: gemma 9B 실측 63GiB 에서 OOM → 78 요구
declare -A WSR_NEED=([llama32_3b]=30 [llama2_7b]=45 [llama31_8b]=50 [qwen25_7b]=50 [gemma2_9b]=78 [llama2_13b]=88); if [ -n "${WSR_NEED_OVERRIDE:-}" ]; then for k in "${!WSR_NEED[@]}"; do WSR_NEED[$k]=$WSR_NEED_OVERRIDE; done; fi
CFGS=("safelora thr0.2 SAFELORA_THRESHOLD=0.2 LORA_ALPHA=16"
      "safelora thr0.25 SAFELORA_THRESHOLD=0.25 LORA_ALPHA=16"
      "wsr_lora rho0.4 KEEP_RATIO=0.4 WSR_LORA_ALPHA=16"
...
log "sweep v6 시작 (허용 GPU: ${ALLOWED_GPUS:-0 1})"
for mk in $MODELS_ORDER; do
  ...
  for cfg in "${CFGS[@]}"; do
    ...
    ALL_DONE=0
    need=${TRAIN_NEED[$mk]}; [ "$m" = wsr_lora ] && need=${WSR_NEED[$mk]}
    g=$(wait_gpu "$need")
  ...
  if [ "${EVAL_PER_MODEL:-1}" = "1" ] && [ ${#REPOS[@]} -gt 0 ]; then run_eval "$mk" "${REPOS[@]}"; else log "   (평가는 마지막 gap-fill 로 미룸)"; fi
done
...
# 1) 7B 이외(HB util ≤0.7 → 73GiB) → 2) 7B(util 0.85 → 88GiB)
NON7=(); SEVEN=(); for r in "${ALL[@]}"; do case "$r" in *llama2_7b-chat*) SEVEN+=("$r");; *) NON7+=("$r");; esac; done
if [ ${#NON7[@]} -gt 0 ]; then g=$(wait_gpu 73); free=$(gpu_free_gib "$g"); cap=0.5; for c in 0.85 0.7 0.6; do [ "$free" -ge "$(awk -v u=$c 'BEGIN{printf "%d", u*97.9+2}')" ] && { cap=$c; break; }; done
  log "── gap-fill(7B 제외) on GPU$g free ${free}GiB LM cap $cap: ${#NON7[@]}개"
  ( cd "$HB" && HB_GPU=$g LM_GPU=$g PARALLEL=0 GPU_UTIL_CAP=$cap RESUME=true LMEVAL_RESUME=1 VALIDATE_REPOS=1 HB_VARIANTS=sys SEED=42 PREFETCH_MODE=blocking ./run_all_eval.sh "${NON7[@]}" ) || log "   ⚠️  gap-fill(7B 제외) 실패 조합 있음"; fi
if [ ${#SEVEN[@]} -gt 0 ]; then g=$(wait_gpu 88); log "── gap-fill(7B) on GPU$g: ${#SEVEN[@]}개"
  ( cd "$HB" && HB_GPU=$g LM_GPU=$g PARALLEL=0 GPU_UTIL_CAP=0.85 RESUME=true LMEVAL_RESUME=1 VALIDATE_REPOS=1 HB_VARIANTS=sys SEED=42 PREFETCH_MODE=blocking ./run_all_eval.sh "${SEVEN[@]}" ) || log "   ⚠️  gap-fill(7B) 실패 조합 있음"; fi
log "════ sweep v6 전체 완료 ════"; echo done > "$REPO/logs/revision_sweep/sweep.rc"
