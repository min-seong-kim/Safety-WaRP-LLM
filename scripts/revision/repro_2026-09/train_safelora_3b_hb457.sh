...
export OUT_ROOT=$PWD/outputs/revision_repro_hb457 PUSH_TO_HUB=0 PRUNE_BASIS=0 PRUNE_HF_CACHE=0
export STAGES="20"
echo "[repro2] start $(date)"; $PY -c "import torch,transformers,peft;print('torch',torch.__version__,'transformers',transformers.__version__,'peft',peft.__version__)"
mkdir -p "$OUT_ROOT"
bash scripts/revision/run_all.sh
rc=$?
echo "[repro2] run_all rc=$rc  $(date)"
echo "$rc" > logs/revision_repro/train2.rc
