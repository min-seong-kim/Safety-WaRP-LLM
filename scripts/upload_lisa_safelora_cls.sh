#!/usr/bin/env bash
#
# run_lisa_safelora_cls.sh 결과물을 HF Hub 에 업로드한다.
#
# repo 이름 규약:
#   ${HF_NS}/llama2_7b-chat-{task}-{method}-r16-a32-lr{LR}-cb
#     task   = sst2 | agnews
#     method = lisa | safelora
#     -cb    = safety data 가 circuit_breakers 임을 표시
#
# 모델 위치가 method 마다 다르다:
#   LISA     → <out_dir>/                (merge_and_unload 후 output_dir 직하)
#   SafeLoRA → <out_dir>/merged_model/   (dense fold 결과)
#
# 사용:
#   bash scripts/upload_lisa_safelora_cls.sh            # LR=3e-4 기본
#   LR=2e-4 bash scripts/upload_lisa_safelora_cls.sh
set -euo pipefail
...
mkdir -p logs
exec > >(tee -a "logs/upload_lisa_safelora_cls_${TS}.log") 2>&1
