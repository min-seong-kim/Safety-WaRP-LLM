...
# 다른 layer
python singular_value_plot_nolog.py --layer 30 --top_k 150 --basis_dir /NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/minseong/Safety-WaRP-LLM/checkpoints/phase1_20260507_004833/basis
...
# 옵션 조합
python singular_value_plot_nolog.py --average --top_k 150 --basis_dir /NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/minseong/Safety-WaRP-LLM/checkpoints/phase1_20260507_004833/basis
...
def plot_and_save_all_layers(top_k=300, target_layer=None, basis_dir=None, average=False):
    if basis_dir is None:
        basis_dir = "/NHNHOME/WORKSPACE/26msit001_A/edge_ai_lab/minseong/Safety-WaRP-LLM/checkpoints/phase1_20260507_004833/basis"
    ...
    if average:
        ...
        for idx, (dir_name, label_name) in enumerate(modules_info):
            ...
            for layer_idx in range(num_layers):
                ...
                S = data.get('S', None) if isinstance(data, dict) else data
                if S is None:
                    continue
                ...
                # 길이를 top_k에 맞추어 패딩 또는 슬라이싱
                S_padded = np.zeros(top_k)
                length = min(len(S), top_k)
                S_padded[:length] = S[:length]
                accumulated.append(S_padded)

            if not accumulated:
                print(f"No data found for module '{dir_name}', skipping.")
                continue

            mean_S = np.mean(accumulated, axis=0)
            x_axis = np.arange(1, top_k + 1)
            plt.plot(x_axis, mean_S, marker=markers[idx], markersize=6,
                     label=label_name, color=colors[idx], linewidth=2,
                     markevery=max(1, top_k // 30))
            any_plotted = True
            print(f"  Module '{label_name}': averaged over {len(accumulated)} layers.")
        ...
        plt.title("Singular Value Distribution (AVG over All Layers)", fontsize=22, fontweight='bold', pad=15)
        ...
        plt.tight_layout()
        save_path = os.path.join(output_dir, "all_layers_average_svd_dist.png")
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"\n✅ 완료! 평균 그래프가 '{save_path}'에 저장되었습니다.")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--average", action="store_true", help="모든 layer의 singular value를 평균내어 하나의 그래프로 저장")
    args = parser.parse_args()
    plot_and_save_all_layers(top_k=args.top_k, target_layer=args.layer, basis_dir=args.basis_dir, average=args.average)